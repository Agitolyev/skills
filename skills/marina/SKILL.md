...
>
> Out of scope: stylistic ktlint, MegaLinter markdown warnings, unrelated changes mixed into the same PR (note them in one line, then skip).
>
> Output: one Markdown table with columns `Severity | File:Line | Finding | Fix`. After the table, a one-to-three sentence Overall. Hard cap at ~600 words. No emojis. No praise. Every finding cites a `file.kt:line`.
...
- Cite `file.kt:line` in every finding. A finding without coordinates is not a finding.
